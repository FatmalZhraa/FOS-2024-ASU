...
inline struct WorkingSetElement* env_page_ws_list_create_element(struct Env* e, uint32 virtual_address)
{
	...
	//create new object of struct WorkingSetElement
	...
	if(e->page_last_WS_element==NULL){
                 //if there is a place ,then initialize it by the given virtual address
			     WS_element->virtual_address=virtual_address;
				 return WS_element;
		}
		else{
			//on failure: panic it
			cprintf("here_5 failed create_ws");
			//panic("Failed to allocate memory WS");
		}
           // if the created object is not null
	       if(WS_element !=NULL){
	        	return WS_element;
	         }
	      else{
	         	cprintf("here_6 failed create_ws");
	             panic("Failed to allocate memory WS");
	            }
