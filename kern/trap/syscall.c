...
void sys_bypassPageFault(uint8 instrLength)
{
 ...
 // Enqueue an environment into the queue
 void sys_enqueue(struct Env_Queue *queue, struct semaphore *sem) {
     enqueue(queue, cur_env);
     //cprintf("we hereeeeeeee\n");
     acquire_spinlock(&ProcessQueues.qlock);
     sem->semdata->lock=0;
     cur_env->env_status = ENV_BLOCKED;
     //cprintf("jjjjjjjjjjjjjj\n");
     sched();
     release_spinlock(&ProcessQueues.qlock);
     //cprintf("hhh\n");
 }
void sys_dequeue(struct Env_Queue* queue) {
	cur_env=dequeue(queue);
    //cprintf("deqqqqq\n");
	cur_env->env_status=ENV_READY;
	acquire_spinlock(&ProcessQueues.qlock);
    //cprintf("deqqqqq 222222\n");
	sched_insert_ready(cur_env);
	 release_spinlock(&ProcessQueues.qlock);
 }
///////////////////////
		//getframinfo (virtual_address , cur_env.envpagedirectory ,uint32* per_page_table)
		//fram=null
		//return frame.sharedobjectid
///////////////////////////////////////
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
   ...
   case SYS_allocate_user_mem:
			if((uint32 *)(a1+a2)==NULL || (a1+a2)>USER_LIMIT )
			{
			   env_exit();
			}
			if((uint32 *)(a1)==NULL || a1>USER_LIMIT )
			{
				 env_exit();
			}
	...
	case SYS_env_set_priority:
		    sys_env_set_priority((uint32) a1, a2);
		    return 0;
			break;
	case SYS_cgetc:
		return sys_cgetc();
