...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
    ...
    uint32 required_size = KERNEL_STACK_SIZE + PAGE_SIZE;


    void* stack_start = kmalloc(required_size);
    if (stack_start == NULL) {
        panic("create_user_kern_stack: Unable to allocate memory for kernel stack");
    }


    uint32 guard_page = (uint32)stack_start;

    //create the stack
    for (uint32 i = 1; i < required_size / PAGE_SIZE; i++) {
        uint32 stack_page = guard_page + (i * PAGE_SIZE);
        struct FrameInfo* frame_info;
        int ret = allocate_frame(&frame_info);
        if (ret != 0 || map_frame(ptr_user_page_directory, frame_info, stack_page, PERM_WRITEABLE) != 0) {
            panic("create_user_kern_stack: Failed to map kernel stack pages");
        }
    }
    return (void*)(guard_page + PAGE_SIZE);
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
    ...
    e->env_start = daStart ;
	e->env_Limit = daLimit ;
	e->env_uh_brk = daStart ;
	uint32 initSizeOfAllocatedSpace = 0;
	initialize_dynamic_allocator(daStart ,initSizeOfAllocatedSpace);
}
