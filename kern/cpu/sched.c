...
void sched_init_PRIRR(uint8 numOfPriorities, uint8 quantum, uint32 starvThresh)
{
	//TODO: [PROJECT'24.MS3 - #07] [3] PRIORITY RR Scheduler - sched_init_PRIRR
		...
		//Create & initialize ready queues
...
struct Env* fos_scheduler_PRIRR()
{
	...
	//TODO: [PROJECT'24.MS3 - #08] [3] PRIORITY RR Scheduler - fos_scheduler_PRIRR
	    struct Env *curProc = get_cpu_proc();
		struct Env *nextProc = NULL;

		//If the curenv is still exist, then insert it again in the ready queue
		if (curProc != NULL)
		{
			enqueue(&(ProcessQueues.env_ready_queues[curProc->priority]), curProc);
		}
	    for (int i = 0; i < num_of_ready_queues; i++)
		{
		        int s = queue_size(&(ProcessQueues.env_ready_queues[i]));
				if(s != 0)
				{
...
void clock_interrupt_handler(struct Trapframe* tf)
{
	...
	if (isSchedMethodPRIRR())
		...
		struct Env* proc ;
		int8 from_highest = num_of_ready_queues - 1 ;
		// loop at queu start from highest
		for (int8 i = from_highest; i >= 0; i--)
		{
				int s = queue_size(&(ProcessQueues.env_ready_queues[i]));
				if(s != 0)
				{
					proc = dequeue(&(ProcessQueues.env_ready_queues[i]));
					//increase ticks's current process
					proc->proc_Ticks++;
					// if process exceeded StarvTH
					if (proc->proc_Ticks > StarvTH)
					{
						//cprintf("tickkkk %d\n ", env->nTicks);
						int promote_proc = i - 1;   	// Promote proc to higher-priority [DECREASE INDX]
						if (promote_proc < 0)
						{
							promote_proc = 0;
						}
						enqueue(&(ProcessQueues.env_ready_queues[promote_proc]), proc);
						//cprintf("Process : %d priority: %d\n",proc->env_id ,promote_proc );
					}
					else
					{
						// if not starved
						enqueue(&(ProcessQueues.env_ready_queues[i]), proc);
					}
				}
			}
