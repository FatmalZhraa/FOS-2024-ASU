...
struct Command commands[] =
{
		...
		{ "schedPRIRR", "switch the scheduler to PRIRR with given # Piriority & quantum & starvThresh" ,command_sched_init_PRIRR, 3},
...
int command_sched_init_PRIRR(int number_of_arguments, char **arguments)
{
	...
	cprintf("Scheduler is now set to PRIRR with %d Priorities & %d quantum & starvThresh = %d\n", numOfPriorities, quantum,starvThresh);
	cprintf("\n");
	return 0;
