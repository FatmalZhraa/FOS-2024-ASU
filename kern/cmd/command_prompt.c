...
int process_command(int number_of_arguments, char** arguments)
{
	//TODO: [PROJECT'24.MS1 - #01] [1] PLAY WITH CODE! - process_command
	 ...
	 LIST_INIT(&foundCommands);
	    ...
	    {
	        char* ac = arguments[0];
	        char* command_copy = commands[i].name;
	        bool length_matches = ( strlen(ac) == strlen(command_copy));
	        bool strings_match = (strcmp(ac, command_copy) == 0);
	        if (length_matches && strcmp(arguments[0], commands[i].name) == 0)
	        {
	            LIST_INIT(&foundCommands);

	            bool arg_count_matches = (number_of_arguments - 1 == commands[i].num_of_args || commands[i].num_of_args == -1);
	        ...
	        while (*ac && strlen(ac) < strlen(command_copy))
	        {
	            if (!strchr(command_copy, *ac))
	                break;

	            command_copy = strchr(command_copy, *ac);
	            ac++;
	        }
	        ...
	        {
	        	are_same = 1;
	            LIST_INSERT_HEAD(&foundCommands, &commands[i]);  // Insert matched command
	        }
	    }
	    if(are_same)
	    	 return CMD_MATCHED ;
