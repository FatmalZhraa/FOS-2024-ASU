...
void _main(void)
{
	...
	cprintf("\n%~[3] Allocate again [test first fit] [50%]\n");
	{
		...
		{ is_correct = 0;
		//cprintf("adresssss : %x\n", ptr_allocations[8]);
		//cprintf("should appear  : %x\n", pagealloc_start + 1*Mega);
	...
	cprintf("\n%~[5] Allocate again [test first fit in coalesced area] [15%]\n");
	{
		...
		if ((uint32) ptr_allocations[13] != (pagealloc_start + 1*Mega + 768*kilo)) {
			//cprintf("adresssss : %x\n", ptr_allocations[13]);
